...
const STATUS_KEY: Record<string, string> = { "on-track": "onTrack", "at-risk": "atRisk", slipped: "slipped", restricted: "restricted" };
const STATUS_DEF_KEY: Record<string, string> = { "on-track": "onTrackDef", "at-risk": "atRiskDef", slipped: "slippedDef", restricted: "restrictedDef" };
...
export function StatusTag({ status, className = "" }: { status: string; className?: string }) {
  const { t: tr } = useLang();
  const label = STATUS_KEY[status] ? tr(STATUS_KEY[status]) : status;
  const def = STATUS_DEF_KEY[status] ? tr(STATUS_DEF_KEY[status]) : label;
  return (
    <Tip as="span" content={def} className={`inline-flex items-center gap-1 text-[11px] ${className}`} style={{ color: statusColor[status], cursor: "help" }}>
