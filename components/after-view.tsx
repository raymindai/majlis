        ...
        <table className="w-full text-[13px]">
          <thead>
            <tr style={{ color: C.faint }}>
              <th className="text-left font-medium pb-1">Owner</th>
          ...
          <tbody>
            {commitments.map((c) => (
              <tr key={c.id} style={{ borderTop: `1px solid ${C.line}` }}>
                <td className="py-2 font-semibold align-top">{c.entity}</td>
                <td className="py-2 align-top pr-3">{c.text}</td>
                <td className="py-2 align-top whitespace-nowrap" style={{ color: C.muted }}>{c.due}</td>
              </tr>
            ))}
          </tbody>
        </table>
